...
public class Sniper : MonoBehaviour
{
    ...
    private IEnumerator FireBurst()
    {
        ...
        for (int i = 0; i < bulletsPerShot; i++)
        {
            ...
            Fire();
            yield return new WaitForSeconds(0.5f); // 연속 발사 간격만큼 대기
        }
    ...
    private void Fire()
    {
        ...
        // SniperBullet 또는 SniperBullet2의 컴포넌트를 가져옵니다.
        if (selectedBulletPrefab == bulletPrefab)
        {
            SniperBullet sniperBullet = bullet.GetComponent<SniperBullet>();
            if (sniperBullet != null)
            {
                sniperBullet.damage = isDoubleDamage ? attackDamage * 2 : attackDamage;
            }
        }
        else if (selectedBulletPrefab == bulletPrefab2)
        {
            SniperBullet2 sniperBullet2 = bullet.GetComponent<SniperBullet2>();
            if (sniperBullet2 != null)
            {
                sniperBullet2.Initialize(attackDamage, isDoubleDamage ? 2f : 1f, isDoubleDamage);
            }
        }
